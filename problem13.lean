...
lemma odd_sq_mod4 (n : ℕ) (hn : n % 2 = 1) : n ^ 2 % 4 = 1 := by
...
lemma mul_lt_pow : ∀ a ≥ 2, ∀ p ≥ 5, p * a < a ^ p := by
  ...
  · rw [show p = p-1+1 by omega, ha, pow_succ, mul_lt_mul_iff_left₀,
      show p-1 = p-2+1 by omega, pow_succ, mul_two, add_assoc]
    apply add_lt_add; apply Nat.lt_pow_self; simp
    rw [show 1+1 = 2^1 by rfl, Nat.pow_lt_pow_iff_right]
    all_goals omega
  rw [show a = a-1+1 by omega, Nat.mul_add_one, add_pow,
    show p+1 = p-1+1+1 by omega]
  ...
  rw [Nat.choose_symm, Nat.choose_one_right]; calc
    _ < (a - 1) ^ (p - 1) * p + (a - 1) ^ p := by
      apply add_lt_add
      · rw [mul_comm, mul_lt_mul_iff_left₀]
...
lemma lm_lt : ∀ n > 1, ∀ p ≥ 5, p * (n + 1) < n ^ p + 1 := by
  ...
  rw [show n+1 = n-2+3 by omega, add_pow, show p+1 = p-1+1+1 by omega]
  repeat rw [sum_range_succ]
  ...
  rw [mul_comm, mul_le_mul_iff_left₀]; calc
    ...
    _ ≤ _ := by gcongr; simp
  any_goals omega
  rw [show p = p-5+5 by omega, pow_add, Nat.add_mul, show 2 ^ 5 = 3 + 29 by rfl,
    Nat.mul_add]
...
-- Assume w. l. o. g. that $a ≤ b$
  ...
  · intro heq; by_cases hp : p = 2
    ...
    -- Factor out the smallest power of $2$ on the RHS of the equation
      let heq' := heq
      rw [hp, ← Nat.ordProj_mul_ordCompl_eq_self a 2, ← Nat.ordProj_mul_ordCompl_eq_self b 2,
        mul_pow, mul_pow, ← pow_mul, ← pow_mul] at heq'
    ...
    -- Prove that the second factor on the LHS of the equation is a power of $2$, denote its power by $k$
      ...
      · exfalso; have keq : k = 0 := by
          ...
          rw [Nat.pow_mod, Nat.mod_self, zero_pow this, Nat.add_mod, Nat.pow_mod,
            Nat.mul_mod] at hk
    ...
    -- Use $a.factorization 2$ to fulfill the goal and prove it satisfies all the desired properties
      ...
      · symm; exact ha
      · symm; rwa [auxeq]
      rw [← ha, ← hb, ← auxeq, hp, ← pow_mul, ← two_mul, ← pow_succ',
        pow_right_inj₀] at heq
      rw [← heq]; ring
      any_goals omega
  ...
  -- Solve the equation when $p ≠ 2$, which is same to say that $p$ is an odd prime
    ...
    rw [← Nat.ordProj_mul_ordCompl_eq_self a p, ← Nat.ordProj_mul_ordCompl_eq_self b p,
      mul_pow, mul_pow, ← pow_mul, ← pow_mul] at heq'
  -- Prove that the multiplicities of $p$ in the prime factorizations of $a$ and $b$ must be equal
    have auxeq : a.factorization p = b.factorization p := by
      ...
      · nth_rw 1 [show b.factorization p = a.factorization p +
        ...
        * (b / p ^ b.factorization p) ^ p ∣ p ^ c := by
        ...
        rw [Nat.pow_mod, Nat.mod_self, zero_pow kpos, Nat.add_mod, Nat.mul_mod] at hk
    ...
    -- Subcase when $b.factorization p < a.factorization p$
      ...
      have kpos : k ≠ 0 := by
        ...
        omega
      apply_fun fun t => t % p at hk
      rw [Nat.pow_mod, Nat.mod_self, zero_pow kpos, Nat.add_mod, Nat.mul_mod, Nat.pow_mod,
        Nat.mod_self, zero_pow] at hk
  ...
  -- Denote the $p$-free part of $a$ and $b$ by $a'$ and $b'$, prove they are positive and $a' < b'$
    ...
    have auxdvd : a' + b' ∣ a' ^ p + b' ^ p := by
      ...
      rwa [Nat.odd_iff]
    have a'ltb' : a' < b' := by
      dsimp [a', b']; rwa [auxeq, Nat.div_lt_div_right]
  ...
  -- Apply the LTE theorem to `hk` to show that $l + 1 = k$
    ...
    rw [padicValNat.prime_pow, padicValNat.pow_add_pow, hl, padicValNat.prime_pow,
      padicValNat_self] at hpv
    rw [← Nat.mul_div_cancel' auxdvd, ← hpv, hl, pow_succ, mul_left_cancel_iff_of_pos,
      ← hl, Nat.div_eq_iff_eq_mul_left] at hk
  -- Prove by contradiction that $a' = 1$
    have a'eq : a' = 1 := by
      ...
      rw [show (a':ℤ)^3+b'^3 = (a'^2+b'^2-a'*b')*(a'+b') by ring, mul_right_cancel_iff_of_pos,
        ← add_sub] at hk
